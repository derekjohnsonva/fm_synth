...
struct EGParameters {
    ...
    attack_time_msec: f32,  // from GUI control
    decay_time_msec: f32,   // from GUI control
    slope_time_msec: f32,   // from GUI control
    release_time_msec: f32, // from GUI control

    // For DXEG
    start_level: f32,        // from GUI control
    end_level: f32,          // from GUI control
    decay_level: f32,        // from GUI control
    sustain_level: f32,      // from GUI control
    shutdown_increment: f32, // Used to shutdown the envelope
}
...
trait EnvelopeGenerator {
    ...
    fn render(&mut self, parameters: &EGParameters, num_samples_to_process: i32) -> f32;
        ...
        for i in 0..num_samples_to_process {
            match self.state {
                ...
                EnvelopeState::NoteOff => {
                    self.output_value += parameters.shutdown_increment;
                    if self.output_value <= MIN_EG_LEVEL {
                        self.output_value = MIN_EG_LEVEL;
                        self.state = EnvelopeState::Off;
                    }
